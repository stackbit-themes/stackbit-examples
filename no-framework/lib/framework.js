...
function updateLayoutsRef() {
  ...
  layouts = Object.fromEntries(
    layoutFiles.map((filePath) => {
      ...
      const layout = fs
        .readFileSync(path.join(LAYOUTS_DIR, filePath), "utf-8")
        .toString();
...
function buildPage(relSrcFilePath) {
  ...
  if (!layout) {
    console.log(
      `[Error] Layout "${page.layout}" not found for page "${relSrcFilePath}"`
    );
    return;
  }
...
if (IS_DEV) {
  ...
  fs.watch(LAYOUTS_DIR, { recursive: true }, (_, filename) =>
    updateSite(filename, LAYOUTS_DIR)
  );
