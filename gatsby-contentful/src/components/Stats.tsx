...
export const Stats = (props: StatsProps) => {
    return (
        <div className={`py-24 px-12 text-center ${themeClassMap[props.theme] ?? themeClassMap['dark']}`} {...objectId(props.contentful_id)}>
...
const StatItem = (props: StatItemProps & { theme: StatsProps['theme'] }) => {
    return (
        <div className={`px-4 py-8 rounded-md ${itemThemeClassMap[props.theme] ?? itemThemeClassMap['dark']}`} {...objectId(props.contentful_id)}>
            <div className="mb-3 text-3xl sm:text-4xl" {...fieldPath('value')}>
                {props.value}
            </div>
            <div className="text-sm uppercase" {...fieldPath('label')}>
                {props.label}
            </div>
        </div>
    );
