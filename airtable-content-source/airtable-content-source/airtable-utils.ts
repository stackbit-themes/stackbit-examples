      ...
      fields: airtableModel.fields.map((airtableField): Field => {
        ...
        switch (airtableField.type) {
          ...
          case 'multipleRecordLinks': {
            const linkedAirtableModel = airtableModels.find(
              (airtableModel) => airtableModel.id === airtableField.options.linkedTableId,
            );
...
function convertAirtableRecordFieldToStackbitDocumentField(
  field: FieldSpecificProps,
  fieldValue: Airtable.FieldSet[keyof Airtable.FieldSet],
): DocumentField {
  switch (field.type) {
    case 'string':
    ...
    case 'list':
      ...
      return {
        type: 'list',
        items: fieldValue.map(
          (item) => convertAirtableRecordFieldToStackbitDocumentField(field.items, item) as DocumentListFieldItems,
        ),
      };
    default:
...
export function convertAirtableAssetRecordsToStackbitAssets(
  records: Airtable.Records<AssetRecordFields>,
  manageUrl: string,
): Asset<AssetContext>[] {
