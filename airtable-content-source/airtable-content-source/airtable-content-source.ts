...
export class AirtableContentSource implements ContentSourceInterface<UserContext, DocumentContext, AssetContext> {
  ...
  async onWebhook(data: {
    data: { base: { id: string }; webhook: { id: string }; timestamp: string };
    ...
    if (webhookData.webhook.id !== this.airtableWebhookId) {
      this.logger.debug(
        `webhook webhook.id '${webhookData.webhook.id}' doesn't match the stored webhook ID ${this.airtableWebhookId}, ignoring webhook`,
      );
      return;
    }
          ...
          for (const [tableId, changedTableData] of Object.entries(payload.changedTablesById)) {
            ...
            if (changedTableData.destroyedRecordIds) {
              if (tableId === this.assetsTableId) {
                ...
                deletedRecordIdPairs.push(
                  ...changedTableData.destroyedRecordIds.map((recordId) => `${tableId}:${recordId}`),
                );
              }
            }
          }
    ...
    if (schemaChanged) {
      ...
      createdAndUpdatedRecordIdPairs.length ||
      createdAndUpdatedAssetIds.length ||
      deletedRecordIdPairs.length ||
      ...
      const assets = convertAirtableAssetRecordsToStackbitAssets(
        updatedAssetRecords,
        `${this.manageUrl}/${this.assetsTableId}`,
      );
  ...
  async getAssets(): Promise<Asset<AssetContext>[]> {
    ...
    const assetRecords = await this.airtableClient.getAirtableRecordsForTable<AssetRecordFields>({
      tableId: this.assetsTableId,
    });
  ...
  async updateDocument(options: {
    ...
    const updatedDocument = convertAirtableRecordsToStackbitDocuments(
      [updatedRecord],
      options.modelMap,
      this.manageUrl,
    )[0];
  ...
  async uploadAsset(options: {
    ...
    const assetDocuments = convertAirtableAssetRecordsToStackbitAssets(
      [assetRecord],
      `${this.manageUrl}/${this.assetsTableId}`,
    );
